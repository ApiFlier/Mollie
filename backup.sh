...
#
# Usage:
#   ./backup.sh           — backup, then optionally offer seed refresh/publish
#   ./backup.sh --quick   — backup only, no seed prompts
#
...
if [ "$QUICK" = true ]; then
    echo ""
    info "Quick backup complete. Run ./backup.sh (without --quick) to also manage seed.sql."
    echo ""
    exit 0
fi
...
case "${SEED_CHOICE}" in
    y|Y)
        ...
        echo ""
        if "$APP_DIR/update-seed.sh"; then
            echo ""
            ...
            if [ -n "$DIRTY_OTHER" ]; then
                ...
                echo ""
                echo "  To publish the seed later:  ./seed.sh  → option [3]"
                echo ""
            else
                ...
                if [[ "${PUB_CHOICE,,}" == "y" ]]; then
                    exec "$APP_DIR/publish-seed.sh"
                else
                    info "Publish skipped."
                    echo ""
                    echo "  To publish later:  ./seed.sh  → option [3] Publish seed"
                    echo "  To review:         ./seed.sh  → option [2] Review diff"
                    echo ""
                fi
            fi
        else
    ...
    *)
        echo ""
        info "Seed not refreshed. Run ./seed.sh any time to manage api/data/seed.sql."
        echo ""
        ;;
esac
